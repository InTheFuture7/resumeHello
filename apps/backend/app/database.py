...
class Database:
    ...
    @property
    def outreach_records(self) -> Table:
        """Outreach history records table."""
        return self.db.table("outreach_records")

    @property
    def outreach_templates(self) -> Table:
        """Outreach template table."""
        return self.db.table("outreach_templates")

    DEFAULT_OUTREACH_TEMPLATE: dict[str, str] = {
        "opening": "您好，我对贵司的{job_title}岗位很感兴趣。",
        "middle_rules": "结合匹配分析，突出简历中与岗位最相关的经历、技能和加分项，保持自然真诚。",
        "closing": "如有机会，希望进一步沟通，谢谢。",
    }
    ...
    def create_outreach_record(
        ...
        """Persist a completed outreach generation as a history record."""
        record_id = str(uuid4())
        now = datetime.now(timezone.utc).isoformat()
        doc: dict[str, Any] = {
            "record_id": record_id,
        ...
        self.outreach_records.insert(doc)
        return doc
    ...
    def list_outreach_records(self) -> list[dict[str, Any]]:
        """Return outreach history ordered newest first."""
        records = list(self.outreach_records.all())
        return sorted(records, key=lambda record: record.get("created_at", ""), reverse=True)

    def delete_outreach_record(self, record_id: str) -> bool:
        """Delete an outreach history record."""
        Record = Query()
        removed = self.outreach_records.remove(Record.record_id == record_id)
        return len(removed) > 0

    def get_outreach_template(self) -> dict[str, Any] | None:
        """Return the single saved outreach template if it exists."""
        templates = self.outreach_templates.all()
        return templates[0] if templates else None

    def save_outreach_template(
        self,
        opening: str,
        middle_rules: str,
        closing: str,
    ) -> dict[str, Any]:
        """Upsert the single active outreach template."""
        Template = Query()
        now = datetime.now(timezone.utc).isoformat()
        existing = self.outreach_templates.all()

        if existing:
            template_id = existing[0]["template_id"]
            self.outreach_templates.update(
                {
                    "opening": opening,
            ...
            result = self.get_outreach_template()
            if result is None:
                raise ValueError("Failed to reload outreach template after update")
            return result
        ...
        self.outreach_templates.insert(doc)
        return doc
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "total_outreach_records": len(self.outreach_records),
            "has_outreach_template": self.get_outreach_template() is not None,
    ...
    def reset_database(self) -> None:
        ...
        self.outreach_records.truncate()
        self.outreach_templates.truncate()
